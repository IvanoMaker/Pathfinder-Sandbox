function backtrack(tile) {
    var paths = [];

    function generate_path(current_tile, current_path) {
        current_path.push(current_tile.pos);
        if (!current_tile.parents || current_tile.parents.length === 0) {
            paths.push(current_path);
        } else {
            for (var i = 0; i < current_tile.parents.length; i++) {
                var new_path = current_path.concat();
                generate_path(current_tile.parents[i], new_path);
            }
        }
    }
    ...
    for (var i = 0; i < tile.parents.length; i++) {
        var current_path = [];
        generate_path(tile.parents[i], current_path);
    }
